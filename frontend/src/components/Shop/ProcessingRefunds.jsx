...
const ProcessingRefunds = () => {
  ...
  useEffect(() => {
    const fetchRefunds = async () => {
      try {
        const { data } = await axios.get(`${server}/withdraw/get-processing-refunds`, {
  ...
  return (
    <div className="w-full p-8">
      <h3 className="text-[22px] font-Poppins mb-4">Processing Payments</h3>
      <div className="bg-white shadow rounded p-4">
        {refunds.length > 0 ? (
          ...
          <p className="text-gray-700">No processing refunds found.</p>
        )}
      </div>
    </div>
  );
...
export default ProcessingRefunds;